...
namespace FundClass {
    ...
    Application& Application::insert(Housing* ptr, double val) {
        ...
        if (find(street, house, door) != -1)
            throw std::invalid_argument("Жилье по такому адресу уже зарегестрировано. Попробуйте еще раз.");
    ...
    Application& Application::save(const std::string& name) {
        ...
        if (!file)
            throw std::invalid_argument("Ошибка открытия/записи файла. Попробуйте еще раз.");
        int sz = table.size();
    ...
    Application& Application::load(const std::string& name)
    {
        ...
        if (!file)
            throw std::invalid_argument("Ошибка открытия/чтения файла. Попробуйте еще раз.");
        int count;
